...
1. `gh run list --workflow agent-loop.yml --limit 6 --json conclusion,createdAt`.
   If the last 3+ concluded runs all failed, inspect one log: "Warning: The
   provided API key is invalid." means CURSOR_API_KEY died; a failure in the
   guard/claim step means the PAT died (check its expiry). Report loudly
   which credential to rotate (see .github/agent/README.md).
